...
int main()
{
  ...
  /* NEW: Setup LidarLites (not sure if working) */
  LidarLiteController();  
...
bool sendResponse(TCPSocket &client)
{
  ...
  // NEW
  response.has_left_lidar = true;
  response.has_center_lidar = true;
  response.has_right_lidar = true;
  ...
  response.left_lidar = static_cast<uint32_t>(1);
  response.center_lidar = static_cast<uint32_t>(2);
  response.right_lidar = static_cast<uint32_t>(3);
